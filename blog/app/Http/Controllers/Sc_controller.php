...
return Redirect::to('/home');
	//return view('home');
        		...
        		return Redirect::back()
                                ->withInput()
                                ->withErrors('Please enter valid username and password');
...
function checkForgotPassword(Request $request){
	 ...
	 $result_check = Sc_users::where('sc_login', '=', $request->input('username'))->get();
        $result = $result_check->toArray();
...
Mail::to($to_email)->send(new ForgotPassword($result[0]));
		...
		return Redirect::to('/')->withInput()
                                ->withErrors('The mail has been sent Successfully');
	...
	return Redirect::back()
                                ->withInput()
                                ->withErrors('That email id does not exists');
                                echo 'error';
}
